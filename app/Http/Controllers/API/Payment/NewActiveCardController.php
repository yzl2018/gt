...
class NewActiveCardController extends ApiController
{
	...
	private $CRM_req_params = [
		...
		'Currency'      	=> '',//货币类型
		'SignType'      	=> '',//签名类型
		'NotifyUrl'			=> ''//商户接收异步通知的地址
	];

	/**
	 * B系统兑换请求参数 携带面值和货币类型 和异步通知地址
	 * @var array
	 */
	private $req_B_params = [
		'mall_id'			=> '',//商城编号
		'merchant_ucode'    => '',//商户ucode
		'notify_url'		=> '',//商城接收异步通知的接口地址
		'ticket'    		=> '',//兑换码
		'secret'    		=> '',//兑换密码
		'amount'    		=> '',//面值
		'currency'  		=> '',//货币类型
	...
	private function doActive(Request $request){
		...
		$is_update = CashCardsModel::where('card_no',$this->CRM_req_params['VoucherNo'])->update(['merchant_notify_url'=>$this->CRM_req_params['NotifyUrl']]);
		if(!$is_update){
			Log::error('update merchant notify url error',[
				$this->CRM_req_params['MerchantCode'] => $this->CRM_req_params['NotifyUrl']
			]);
			return $this->error_message('07');
		}
...
//		}
	...
	private function build_params_b(){
		...
		$this->req_B_params['mall_id'] = config('system.mall.code');
		$this->req_B_params['notify_url'] = config('system.mall.receive_url');
		if(isset($this->CRM_req_params['CRMOrderNo'])){
			$this->req_B_params['cid'] = $this->CRM_req_params['CRMOrderNo'];
		}
		$this->req_B_params['merchant_ucode'] = MerchantSecurityModel::where('merchant_code',$this->CRM_req_params['MerchantCode'])->value('user_code');
	...
	public function dispose_response($resp){
		...
		if(is_null(json_decode($resp))){
			...
			if($res->code == '0'){
				...
				try{
					$redis_key = $this->dispatchMailJob(MAIL::card_active_success,[
				...
				$result['VoucherValue'] = $this->CRM_req_params['VoucherValue'];
				$result['Currency']	= $this->CRM_req_params['Currency'];
